package com.tongtong.springmvc.servlet;
...
import com.tongtong.springmvc.annotation.TTService;

public class TTDispatcherServlet extends HttpServlet{
		...
		try {		
			for(String className:classNames){
				...
				if(clazz.isAnnotationPresent(TTController.class)){
					...
					TTService service = clazz.getAnnotation(TTService.class);
		...
		private void putParamIndexMapping(Method method) {
			...
			for(int i=0;i<pa.length;i++){
				for(Annotation a:pa[i]){
					if(a instanceof TTRequestParam){
						String paramName = ((TTRequestParam)a).value();
						if(!"".equals(paramName)){
